...
### D30: B4 waste/risk + live memo route architecture, the two controls, and the value-tag wiring (2026-06-09)
- **Decision (the route receives computed inputs, not raw events):** The live memo route at `app/api/memo/route.ts` accepts a compact, code-computed `MemoInputs` object (summary metrics, the eligible cost-driver list, the needs-review exclusions, the quantified risk flags, the B3 budget report, and the forecast), NOT the raw canonical events. The numbers are produced by the same deterministic pure functions the dashboard uses (`lib/memo/build-inputs.ts` `buildMemoInputs`), run in the browser, so the AI still computes nothing (DISCOVERY section 4). The chief reason raw events are not sent: Vercel caps a serverless request body at ~4.5 MB, and D15 allows up to 50k rows, which would exceed it; the aggregated inputs are tiny. The route then derives the C2 supplied-figure set server-side FROM the received inputs (`collectMemoFigures`), so the validation set is exactly what the prompt was shown and is never trusted from a separate client field.
- **Decision (the model writes language, code injects every number):** The model returns only a `MemoNarrative` (prose fields: executive summary, per-section comments, ordered recommendations, questions, a data-sufficiency line) via Anthropic structured outputs (`output_config.format` json_schema), and `assembleMemo` composes that prose with the code-computed numbers into the rendered `MemoDocument`. Every dollar shown in headline, driver table, flags, budget list, and forecast list is app-injected from a supplied figure; the model never emits a structured number. This is the strongest reading of the architectural law and keeps C2 mostly guarding the model's prose. The memo reuses MemoView's existing block types (paragraph, table, flags, list), so MemoView was NOT edited.
- **Decision (control C1, eligibility, in code):** `lib/metrics/eligibility.ts` `partitionDrivers` decides needs-review in code before the prompt is assembled: a driver is eligible only with an attributable owner AND at least `MIN_EVENTS` (3) events AND at least `DOLLAR_FLOOR` ($50) of spend; everything else is excluded with a specific reason and handed to the model tagged "exclude". The floors are a one-line switch (`DEFAULT_ELIGIBILITY`). Golden test (`eligibility.test.ts`) proves both cases: a well-supported driver is eligible, and a thin-event / ownerless / sub-floor driver never reaches the eligible set.
- **Decision (control C2, number integrity, in the route):** `lib/memo/validate.ts` `sanitizeMemo` scans every prose string in the assembled memo for dollar tokens and strips any that do not trace (within rounding tolerance, max($1, 0.5%)) to a supplied figure, replacing it with a withheld marker and reporting what was removed; the route applies it before returning, so no unsupported figure survives. Only dollar figures are validated (per the control wording); percentages and counts are app-injected. Test (`validate.test.ts`) proves both cases: a supported figure survives untouched, a fabricated figure is flagged and stripped. A self-inflicted false positive was caught and fixed during the batch: the C1 exclusion reason embedded the literal "$50" floor, which C2 then flagged, so the reason was reworded to "below the 50-dollar attribution floor" (no "$" token, because the floor is a gate, not a memo figure).
- **Decision (model-tier repricing, D12):** `lib/metrics/risk.ts` carries a static, editable `MODEL_TIERS` map (provider+model into frontier/mid/cheap with a recommended cheaper target), tiers aligned to the Northstar sample so the live memo reconciles with the dashboard. "Expensive-model misuse" savings = current cost of low-value spend on a frontier model minus the same tokens repriced at the recommended cheaper target (via `selectPrice` + `deriveCost`), counting only events whose target model is priced. This is the honest savings figure that supersedes citing the raw frontier-on-low-value spend as the impact (the cached B1 memo used the spend; the live memo uses the savings). `computeRiskFlags` emits frontier-misuse, low-value, unapproved, missing-owner, and (when present) usage-spike flags, each with a positive dollar impact and a code-suggested control. Risk-flag `detail` strings deliberately carry no "$" amounts (the dollar is `impact_usd`) so they never trip C2.
- **Decision (B3 budget + forecast folded into the memo, D26 closed):** `buildMemoInputs` folds the B3 `buildBudgetReport` and `buildOutlook` into the inputs. A budget section renders only when the dataset carries budgets; uploaded data has none, so the memo frames "no budget framework yet". The cached hero memo (`data/precomputed-memo.json`) is kept AS-IS (it is the committed, reconciled baseline-review artifact, D26); the LIVE route is where budgets/forecast/repricing appear on data that carries them. `build-inputs.test.ts` proves the fold on the real Northstar set (org status overrun, frontier-misuse savings strictly below the frontier-low-value spend) and runs C1 + C2 together on the actual numbers.
- **Decision (Haiku value-tag suggestion WIRED, D6/D27 closed):** Rather than defer a third time, the Haiku classifier is wired this batch as a second action on the same route (`{ action: "classify", labels }`), with a "Suggest value tiers" button in `components/mapping-editor.tsx`. The deterministic rule map stays the source of truth; Haiku only suggests a tier for the listed actors (classification, never computation) and every suggestion remains editable (D6). One server route, one key, both models server-side.
- **Decision (models, D16):** memo = `claude-opus-4-8` with adaptive thinking, `effort: "high"`, structured outputs, `max_tokens: 8000` (one-line switches at the top of `prompt.ts`/`route.ts`); classifier = `claude-haiku-4-5` with structured outputs and no thinking/effort (effort errors on Haiku 4.5). Model IDs and SDK params taken from the `claude-api` skill, not memory; `@anthropic-ai/sdk@0.102.0` types confirmed to carry `output_config.format`, `effort`, and adaptive `thinking`.
- **Decision (route security, light weight class):** the key is read from `process.env.ANTHROPIC_API_KEY` server-side and never reaches the client; the route is rate-limited (8 requests/IP/minute, in-memory, best-effort, resets on cold start, documented as a demo guard not a hard quota); the body is capped at 1 MB; `MemoInputs` is fully reconstructed and sanitized from untrusted JSON (array caps, control-char stripping, string length caps) so an uploaded CSV cannot smuggle instructions into the prompt; the system prompt tells the model the data values are data, not instructions; the output is schema-constrained so injected text cannot change app behavior; and typed Anthropic errors are mapped to clean status codes that never echo the key or detail (an `AuthenticationError` returns the generic "not configured").
- **Files touched beyond the literal B4 bounded list (recorded per D17):** `app/upload/page.tsx` (wire the RiskView and the GenerateMemo control onto the upload surface, the natural home for the live-upload memo, D3; the prompt anticipated this), `lib/memo/build-inputs.test.ts` (the C1+C2-on-real-data integration proof), and `e2e/memo-live.spec.ts` (the rung-3 check, `/api/memo` mocked so CI is deterministic). `components/mapping-editor.tsx` is within `components/*` (the Haiku wiring). The cached memo, the dashboard, and MemoView were not touched.
- **Verification:** rung 2 (Vitest: 104 tests, 13 files, including the C1 golden, the C2 both-cases, the D12 repricing hand-check, and the real-data integration) PLUS rung 3 (Playwright: 16, including the two new memo-live cases) PLUS rung 4 (live end to end against the real Claude API: the memo route returned HTTP 200 with `validation.ok: true` and an executive summary that used only the supplied figures, and the Haiku classify action returned tier suggestions). Build, lint, typecheck clean; diff scanned for secrets before commit.
- **Traded off:** a larger B4 surface (two pure controls, the inputs/prompt/validate trio, the server route, two components, the upload wiring, the Haiku second action, four test files) for the full credibility loop (a live, validated CFO memo on real uploaded data, with both honesty controls enforceable and tested) and for closing D6/D27 rather than deferring again. The route receives pre-aggregated inputs rather than raw events, accepted because it stays within the Vercel body limit and keeps the AI out of computation either way.
- **Confidence:** Inferred-high (architecture, thresholds, the tier map, and the security posture are reversible and resolvable from D3/D6/D12/D13/D16/D26 plus the `claude-api` skill, without a new user call). **Status:** Active.
- **Flags for the human:** (1) `PRICING_CONFIRMED` is still false (D11/D28), so the repricing savings magnitude is illustrative until the pricing values are confirmed; (2) the builder's real Anthropic export is still the open rung-4 "real numbers" box (D9/D28), now also exercisable through the live memo on `/upload`; (3) the eligibility floors ($50, 3 events) and the `MODEL_TIERS` map are sensible defaults open to tuning; (4) rate limiting is best-effort in-memory (a real quota would need persistence, deferred with D2). **Local note (machine-specific, not committed):** `next start` does NOT auto-load `.env.local`, so the local rung-4 run required exporting `ANTHROPIC_API_KEY` into the server process, and the Anthropic SDK's outbound TLS needed `NODE_OPTIONS=--use-system-ca` behind the corporate proxy (same root cause as the npm and git workarounds). Vercel sets the env var directly and is not behind the proxy, so neither applies in production.
